(function () {
  ...
  function fuzzyMatch(haystack, query) {
    ...
    // fuzzy: every char of query appears in order in haystack
    let hi = 0;
    for (let qi = 0; qi < query.length; qi++) {
      hi = haystack.indexOf(query[qi], hi);
  ...
  function highlightText(node, query) {
    ...
    if (!h3) return;
    const original = h3.dataset.originalText || h3.textContent;
    h3.dataset.originalText = original;
    if (!query) { h3.textContent = original; return; }
    const lower = original.toLowerCase();
    const qLower = query.toLowerCase();
    // substring highlight only (fuzzy highlight is too noisy)
    const idx = lower.indexOf(qLower);
    if (idx < 0) { h3.textContent = original; return; }
    h3.innerHTML =
      escapeHtml(original.slice(0, idx)) +
      "<mark>" + escapeHtml(original.slice(idx, idx + query.length)) + "</mark>" +
    ...
    inputs.forEach((input) => {
      ...
      function applyFilter() {
        ...
        items.forEach((item) => {
          ...
          if (match) {
            visible += 1;
            highlightText(item, input.value.trim());
          } else {
            // restore original text
            const h3 = item.querySelector("h3");
            if (h3 && h3.dataset.originalText) h3.textContent = h3.dataset.originalText;
          }
        });
        if (countNode) {
