...
> 2026-08-27 第二轮验收整改（引语级修正与审计语义收敛）完成，本批仍无新增阻塞。

## B-3 第四批A收尾：既有测试与执行后数据语义冲突（待授权）

- 生产裁决、覆盖率、发布层、静态站和允许范围内的回归测试均已通过；全量 pytest 当前为 **72 passed / 2 failed**。
- `tests/test_batch4a_candidate_audit.py` 仍要求候选审计快照的 14 个证据 ID 必须等于执行后的 5 事件生产证据集合。第四批A按授权删除两条事实、删除 EVT-00005，并把 FE-EVP3-0005 改挂 EVT-00004 后，执行后集合天然不再是原始 14 条，因此该既有断言失败；审计表本身必须保留 14 行作为永久审计留档。
- `tests/test_merge_batch3_review.py` 的 fresh-add 用例从当前生产表取样，却硬编码第三批合并后事件数为 148。第四批A已按授权将生产事件数降为 147；该用例应改为固定 30d5f77/第四批前基线或改成基线驱动，不能让第三批重放流程新建占位事件。
- 上述两个测试文件不在本任务白名单内。当前未擅自修改、跳过或屏蔽测试；需要明确授权后才能做最小化测试兼容修正。除该白名单冲突外，本批没有新的史料或数据语义阻塞。

> 2026-08-27 第五批（第四批A裁决落地）：经验收补充授权修复两个既有测试（candidate_audit 基线锚定 2a87781、batch3_fresh 动态基数），全量 74 passed。本批无新增阻塞。
