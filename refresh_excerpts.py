...
class ProjectPaths:
    ...
    def discover(cls, root: Path) -> "ProjectPaths":
        input_dir = next(p for p in root.iterdir() if p.is_dir() and p.name.startswith("input_"))
        raw_text_dir = next(p for p in input_dir.iterdir() if p.is_dir() and p.name.startswith("raw_texts_"))
        output_dir = next(p for p in root.iterdir() if p.is_dir() and p.name.startswith("output_"))
        kb_dir = next(p for p in output_dir.iterdir() if p.is_dir() and p.name.startswith("kb_data_"))
        app_dir = root / next(p.name for p in root.iterdir() if p.is_dir() and p.name.startswith("knowledge_base_")) / "app"
        work_dir = next(p for p in root.iterdir() if p.is_dir() and p.name.startswith("work_"))
        extracted_dir = next(p for p in work_dir.iterdir() if p.is_dir() and p.name.startswith("extracted_"))
