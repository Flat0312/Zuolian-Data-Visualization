...
def _load_source_page_index(base_dir_text: str) -> dict[str, dict[int, str]]:
    ...
    runtime_dir = project_root / "data" / "processed" / "runtime_sources"
    return {
        "左联词典": _parse_paged_txt(runtime_dir / "左联词典.txt"),
        "左联史": _parse_paged_txt(runtime_dir / "左联史.txt"),
        "左联回忆录": _parse_paged_json(runtime_dir / "左联回忆录_ocr_text.json"),
    }
