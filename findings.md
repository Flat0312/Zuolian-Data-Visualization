...
- 组织身份事实级证据覆盖率为 100%；第三批合并后的历史快照为 28/148，第四批A按人工授权清除错挂并删除重复事件后，当前事件存在覆盖率为 17.7%（26/147，2026-08-27）。覆盖率报告当前区分三种事件口径：已挂接 26/147、直接支持 21/147、已确认 26/147，三者均排除 `review_status=rejected` 且语义互不替代；人物生卒年和角色仍缺少事实级证据。
- 4238 条人物关系中，1974 条为 critical 风险，关系研究结论必须结合人工审核与证据等级解释。
- 网络包含 41 个连通分量；最大连通分量为 57 人。中心性结果可能受到资料收集偏差影响，不能直接等同于历史重要性。
- 事件与地点审核队列当前保留 160 条，说明现有数据可用于研究探索，但仍需要持续人工复核。
